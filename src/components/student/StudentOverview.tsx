...
interface StudentOverviewProps {
  ...
  onNavigateForm: () => void;
  whatsappLink?: string;
  setActiveTab: (tab: 'dashboard' | 'permohonan' | 'pengaturan' | 'laporan' | 'kelulusan') => void;
...
export default function StudentOverview({
  ...
  const isProfileComplete = !!(currentUser.universitas && currentUser.prodi && currentUser.noHp);
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="tab-content-dashboard">
      ...
      <section className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6" id="student-profile-hero">
          ...
          <div className="flex-1 space-y-2">
            ...
            {isProfileComplete ? (
              <div className="pt-1 text-center md:text-left">
                <p className="text-xs text-blue-600 font-semibold">{currentUser.universitas} • {currentUser.prodi}</p>
                <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1.5 text-[11px] text-slate-500 mt-2 font-medium">
                  ...
                  <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {currentUser.noHp}</span>
                </div>
              </div>
            ) : (
              <div className="pt-1 flex flex-col items-center md:items-start">
                <div className="p-3 bg-amber-50 border border-amber-200 text-amber-800 text-[11px] font-semibold rounded-2xl flex items-start gap-2 max-w-xl text-left">
                  <AlertTriangle className="h-4.5 w-4.5 text-amber-600 shrink-0 mt-0.5" />
                  <span>Profil Anda belum lengkap. Mohon lengkapi terlebih dahulu profil Anda sebelum melakukan pendaftaran magang.</span>
                </div>
                <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1.5 text-[11px] text-slate-500 mt-2.5 font-medium">
                  <span className="flex items-center gap-1"><Mail className="h-3 w-3" /> {currentUser.email}</span>
                </div>
              </div>
            )}
      ...
      {!application ? (
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 md:p-12 text-center space-y-6 shadow-xs max-w-xl mx-auto" id="dashboard-empty-state">
          ...
          <div className="space-y-2">
            <h4 className="font-display font-extrabold text-lg text-slate-900">Pendaftaran Belum Tersedia</h4>
            <p className="text-slate-500 text-xs md:text-sm leading-relaxed max-w-md mx-auto">
              Anda belum melakukan pendaftaran magang di portal MAGIKA. Mulailah perjalanan karir Anda dengan mendaftar sekarang.
            </p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8" id="dashboard-track-state">
          {/* Left Column: Progress status log */}
          <div className={`${application.status === 'Ditolak' ? 'lg:col-span-12' : 'lg:col-span-8'} space-y-6`}>
            <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Status Berkas Permohonan</span>
                  <div className="text-xs text-slate-500 font-semibold mt-0.5">ID Pendaftaran: <span className="font-mono text-blue-600 font-bold">{application.id}</span></div>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="h-6 w-6 bg-emerald-500 text-white rounded-full flex items-center justify-center text-xs font-bold">✓</div>
                ...
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      application.status !== 'Menunggu' ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white animate-pulse'
                    }`}>
                      {application.status !== 'Menunggu' ? '✓' : '2'}
                    </div>
                    <div className={`h-12 w-[2px] ${
                      application.status === 'Lulus' || application.status === 'Ditolak' ? 'bg-emerald-500' : 'bg-slate-200'
                    }`} />
                  </div>
                  <div>
                    <h5 className="font-bold text-slate-800 text-xs">Verifikasi Kelengkapan Berkas</h5>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-mono">Status: {application.status}</p>
                    <p className="text-xs text-slate-500 mt-1">
                      {application.status === 'Menunggu' && 'Berkas Anda sedang masuk dalam antrean peninjauan petugas Kecamatan Cicalengka.'}
                      {application.status === 'Lulus' && 'Berkas dinyatakan lengkap, valid, dan memenuhi kriteria penerimaan.'}
                      {application.status === 'Ditolak' && 'Verifikasi selesai dengan beberapa kendala administrasi.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      application.status === 'Lulus' ? 'bg-emerald-500 text-white' : application.status === 'Ditolak' ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-400'
                    }`}>
                ...
                <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl space-y-3">
                  <div className="flex items-start gap-2 text-rose-800 text-xs font-bold">
                    ...
                    <div>
                      ...
                      <p className="font-normal mt-1 text-rose-900">{application.rejectionReason || 'Berkas Anda belum sesuai ketentuan.'}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {application.status === 'Lulus' ? (
                <>
                  ...
                  <div className="bg-emerald-600 text-white p-5 rounded-3xl space-y-3 shadow-lg shadow-emerald-500/10">
                    ...
                    <h5 className="font-bold text-sm">Grup WhatsApp Koordinasi</h5>
                    <p className="text-[11px] text-emerald-100 leading-relaxed">
                      Pendaftar yang diterima wajib bergabung ke grup WhatsApp resmi koordinasi untuk koordinasi pembagian seksi dan magang.
                    </p>
                    <a 
                      href={whatsappLink || "https://chat.whatsapp.com/mock-magika-cicalengka"} 
                      target="_blank" 
                      rel="noreferrer"
                  ...
                  {application.kategoriPendaftar !== 'siswa' && (
                    <div className="bg-white border border-slate-200 rounded-3xl p-5 space-y-3.5 shadow-xs">
                      <h6 className="text-xs font-bold text-slate-800">Unduh Dokumen Kelulusan</h6>
                      ...
                      >
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <div>
                            <div className="font-bold text-slate-700">Surat Kelulusan Resmi</div>
                            <p className="text-[9px] text-slate-400 mt-0.5 uppercase font-semibold">Ditandatangan Camat • PDF</p>
                          </div>
                        </div>
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-xs text-xs">
                  <h6 className="font-bold text-slate-800">Panduan Verifikasi Administrasi</h6>
                  <p className="text-slate-500 leading-relaxed text-xs">
                    Petugas kami akan memeriksa kecocokan data Anda bersama 2 berkas lampiran pendukung.
                  </p>
                  <div className="space-y-3 pt-2">
                    <div className="flex gap-2.5 items-start">
                      <span className="h-4 w-4 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center font-bold font-mono text-[10px] shrink-0">1</span>
                      <p className="text-slate-600">Pastikan nomor handphone WhatsApp aktif untuk koordinasi lanjutan jika diperlukan perbaikan berkas.</p>
                    </div>
                    <div className="flex gap-2.5 items-start">
                      <span className="h-4 w-4 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center font-bold font-mono text-[10px] shrink-0">2</span>
                      <p className="text-slate-600">Lacak hasil kelulusan secara langsung dari halaman ini dalam waktu 3-7 hari kerja ke depan.</p>
                    </div>
                  </div>
                </div>
              ) : null}
