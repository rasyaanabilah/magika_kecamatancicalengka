...
export default function StudentDashboard({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans selection:bg-blue-500 selection:text-white" id="student-portal-container">
      
      {/* DESKTOP SIDEBAR - Hidden on mobile, sticky on desktop */}
      <aside className="w-64 bg-slate-900 text-slate-400 hidden lg:flex flex-col justify-between shrink-0 sticky top-0 h-screen overflow-y-auto border-r border-slate-800" id="student-desktop-sidebar">
        <div className="p-6 space-y-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Portal Peserta</div>
            </div>
          </div>
          ...
          <nav className="space-y-1 text-xs">
            ...
            <button 
              ...
              className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                activeTab === 'dashboard' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
              }`}
            >
            ...
            <button 
              ...
              className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                activeTab === 'permohonan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
              }`}
            >
            ...
            {application && application.status === 'Lulus' && application.kategoriPendaftar !== 'siswa' && (
              <button 
                onClick={() => setActiveTab('kelulusan')}
                className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                  activeTab === 'kelulusan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
                }`}
              >
            ...
            <button 
              ...
              className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                activeTab === 'pengaturan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
              }`}
            >
      ...
      <header className="h-16 border-b border-slate-200 bg-white px-4 md:px-8 flex lg:hidden items-center justify-between shrink-0" id="student-header">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display font-extrabold text-base text-slate-900 leading-none">MAGIKA</div>
            <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Portal Peserta</div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 flex lg:hidden" id="student-drawer-container">
            ...
            >
              <div className="space-y-8">
                ...
                <nav className="space-y-1.5 text-xs">
                  ...
                  {/* Berkas Permohonan */}
                  <button 
                    onClick={() => { setActiveTab('permohonan'); setIsMenuOpen(false); }}
                    className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                      activeTab === 'permohonan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
                    }`}
                  >
                  ...
                  {application && application.status === 'Lulus' && application.kategoriPendaftar !== 'siswa' && (
                    <button 
                      onClick={() => { setActiveTab('kelulusan'); setIsMenuOpen(false); }}
                      className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                        activeTab === 'kelulusan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
                      }`}
                    >
                  ...
                  {application && application.status === 'Lulus' && (
                    <button 
                      onClick={() => { setActiveTab('laporan'); setIsMenuOpen(false); }}
                      className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                        activeTab === 'laporan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
                      }`}
                    >
                  ...
                  {/* Profil & Pengaturan */}
                  <button 
                    onClick={() => { setActiveTab('pengaturan'); setIsMenuOpen(false); }}
                    className={`w-full px-4 py-3 rounded-xl font-semibold flex items-center gap-3 transition-colors cursor-pointer ${
                      activeTab === 'pengaturan' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-slate-200'
                    }`}
                  >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <main className="flex-1 p-4 md:p-8 overflow-y-auto space-y-8" id="student-main-content">
