...
export default function StudentGraduation({
  ...
  const suratListForStudent = studentSuratList.length > 0 
    ? studentSuratList 
    : (studentSurat ? [studentSurat] : []);

  const suratKeterangan = suratListForStudent.find(s => s.tipeSurat === 'keterangan_magang');
  const suratBalasan = suratListForStudent.find(s => s.tipeSurat === 'balasan');
  ...
  const handlePrintKeteranganMagang = (targetSurat?: any) => {
    ...
    printLetter({
      ...
      namaPeserta: s?.namaPeserta || application?.namaLengkap || currentUser.namaLengkap,
      nimNisn: s?.nimNisn || application?.nim || application?.nisn || currentUser.prodi || '-',
      prodiJurusan: s?.prodiJurusan || application?.prodi || application?.jurusan || currentUser.prodi || '-',
      instansiPendidikan: s?.instansiPendidikan || application?.universitas || currentUser.universitas || 'Universitas Ma\'soem',
      tanggalMulai: s?.tanggalMulai || application?.tanggalMulaiMagang || application?.tanggalMulai || '1 Juli 2026',
      tanggalSelesai: s?.tanggalSelesai || application?.tanggalSelesaiMagang || application?.tanggalSelesai || '31 Agustus 2026',
      penandatanganNama: s?.penandatanganNama || 'Neni Runingdiyah, S.Kom',
      penandatanganNip: s?.penandatanganNip || '19810924 201004 2 001',
      penandatanganJabatan: s?.penandatanganJabatan || 'Kasubag Umum dan Kepegawaian',
  ...
  const handlePrintBalasan = (targetSurat?: any) => {
    ...
    if (s?.daftarPesertaSurat) {
      ...
      list = [{
        ...
        instansi: application?.universitas || '-'
      }];
    }

    printLetter({
      tipeSurat: 'balasan',
      nomorSurat: s?.nomorSurat || application?.suratPengantarNo || '400.14.5.4/270/Sekret',
      tanggalKeluar: s?.tanggalKeluar || application?.suratPengantarTanggal || '31 Maret 2026',
      lampiran: s?.lampiran || application?.suratPengantarLampiran || '-',
      perihal: s?.perihal || application?.suratPengantarPerihal || 'Balasan Permohonan Izin Praktik Adaptasi Lapangan',
      sifat: s?.sifat || application?.suratPengantarSifat || 'Penting / Segera',
      penandatanganNama: s?.penandatanganNama || application?.suratPenandatanganNama || 'CUCU HIDAYAT, S.H., M.M.',
      penandatanganJabatan: s?.penandatanganJabatan || application?.suratPenandatanganJabatan || 'CAMAT',
      penandatanganNip: s?.penandatanganNip || application?.suratPenandatanganNip || '19710731 199811 1 001',
      kepadaJabatan: s?.kepadaJabatan || application?.suratKepadaJabatan || 'Dekan / Pimpinan',
      kepadaInstansi: s?.kepadaInstansi || application?.suratKepadaInstansi || application?.universitas || 'Instansi Pendidikan',
      tempat: s?.tempat || application?.suratTempat || 'Tempat',
      rujukanPengirim: s?.rujukanPengirim || application?.rujukanPengirim || 'Dekan / Pimpinan',
      rujukanInstansi: s?.rujukanInstansi || application?.rujukanInstansi || application?.universitas || 'Instansi',
      rujukanNo: s?.rujukanNo || application?.rujukanNo || '267/FKOM-UM/III/2026',
      rujukanTgl: s?.rujukanTgl || application?.rujukanTgl || '30 Maret 2026',
      rujukanPerihal: s?.rujukanPerihal || application?.rujukanPerihal || 'Izin Praktik Adaptasi Lapangan',
      isiSurat: s?.isiSurat || application?.suratPengantarIsi || `Sehubungan hal tersebut, pada prinsipnya kami tidak berkeberatan yang bersangkutan Melakukan Praktik Adaptasi Lapangan terhitung tanggal ${application?.tanggalMulai || '1 Juli 2026'} Sampai ${application?.tanggalSelesai || '28 Juli 2026'} sepanjang memenuhi persyaratan normatif.`,
      tembusan: s?.tembusan || application?.suratTembusan || '1. Kepala Badan Kesbangpol Kabupaten Bandung.',
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto" id="tab-content-kelulusan-wrapper">
      {/* Container */}
      <div className="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 shadow-xs space-y-6 font-sans" id="tab-content-kelulusan">
        <div className="space-y-1 border-b border-slate-100 pb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-extrabold">
            <Award className="h-3.5 w-3.5 text-blue-600" /> Dokumen Kelulusan Magang
          </div>
          <h3 className="font-display font-extrabold text-xl text-slate-900 leading-tight">
            Dokumen & Surat Kelulusan Resmi
          </h3>
          <p className="text-xs text-slate-500">
            Cetak dan unduh Surat Keterangan Magang Kerja (Kelulusan) serta Surat Balasan Permohonan Resmi dari Kecamatan Cicalengka.
          </p>
        </div>
          ...
          <div className="space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 hover:border-emerald-300 transition-all">
                <div className="space-y-1 text-center sm:text-left flex-1">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-emerald-100 text-emerald-800 font-extrabold text-[10px]">
                    <FileCheck className="h-3 w-3 text-emerald-600" /> Surat Kelulusan Magang
                  </div>
                  <h4 className="font-bold text-slate-900 text-base">
                    Surat Keterangan Magang Kerja
                  </h4>
                  <p className="text-xs text-slate-500">
                    Surat bukti resmi bahwa Anda telah menyelesaikan kegiatan magang kerja di Kecamatan Cicalengka.
                  </p>
                </div>
                <button 
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 hover:border-blue-300 transition-all">
                <div className="space-y-1 text-center sm:text-left flex-1">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-blue-100 text-blue-800 font-extrabold text-[10px]">
                    <FileCheck className="h-3 w-3 text-blue-600" /> Surat Penerimaan / Izin
                  </div>
                  <h4 className="font-bold text-slate-900 text-base">
                    Surat Balasan Permohonan Magang
                  </h4>
                  <p className="text-xs text-slate-500">
                    Surat jawaban resmi atas surat permohonan izin magang dari kampus/sekolah Anda.
                  </p>
                </div>
                <button 
            ...
            {!suratKeterangan && !suratBalasan && (
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 hover:border-blue-300 transition-all">
                <div className="space-y-1 text-center sm:text-left flex-1">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-blue-100 text-blue-800 font-extrabold text-[10px]">
                    <FileCheck className="h-3 w-3 text-blue-600" /> Surat Resmi Kecamatan
                  </div>
                  <h4 className="font-bold text-slate-900 text-base">
          ...
          <div className="bg-amber-50/80 border border-amber-200/90 rounded-2xl p-8 text-center space-y-3">
            ...
            <div className="space-y-1 max-w-md mx-auto">
              ...
              <p className="text-slate-600 text-xs leading-relaxed font-medium">
                Mohon tunggu dan tinjau terus dokumen kelulusan. Petugas Kecamatan Cicalengka belum mengirimkan surat resmi untuk akun Anda.
              </p>
            </div>
          </div>
        )}
