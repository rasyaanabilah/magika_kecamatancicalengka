...
export const CamatDashboardTab: React.FC<CamatDashboardTabProps> = ({
  ...
  const filteredApps = applications.filter((app) => {
    const instansiText = (app.instansiPendidikan ?? "").toLowerCase();
    const matchesSearch =
      app.namaLengkap.toLowerCase().includes(searchTerm.toLowerCase()) ||
      app.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      instansiText.includes(searchTerm.toLowerCase());
    const matchesStatus =
      ...
      <div className="bg-gradient-to-r from-blue-600 to-emerald-600 rounded-3xl p-6 md:p-8 text-white shadow-xl shadow-blue-900/10 relative overflow-hidden flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="relative z-10 space-y-2 flex-1 text-center md:text-left">
          <h1 className="font-display text-xl md:text-2xl font-black tracking-tight">
            Selamat Datang,{" "}
            {currentUser?.namaLengkap || currentUser?.email || "Camat"}
          </h1>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            ...
            <span className="text-xl font-extrabold font-mono text-slate-900 block mt-1">
              {totalCount}
            </span>
          </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            ...
            <span className="text-xl font-extrabold font-mono text-emerald-600 block mt-1">
              {acceptedCount}
            </span>
          </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            ...
            <span className="text-xl font-extrabold font-mono text-blue-600 block mt-1">
              {pendingCount}
            </span>
          </div>
          <div className="h-9 w-9 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center shrink-0">
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            ...
            <span className="text-xl font-extrabold font-mono text-rose-600 block mt-1">
              {rejectedCount}
            </span>
          </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            ...
            <span className="text-xl font-extrabold font-mono text-blue-800 block mt-1">
              {applicationsWithReports.length}
            </span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Lakukan audit berkas pendaftaran dan pemantauan penempatan magang
              peserta.
            </p>
          </div>
