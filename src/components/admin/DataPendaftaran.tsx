...
export default function DataPendaftaran({
  ...
  const [formJenisKelamin, setFormJenisKelamin] = useState<
    "Laki-laki" | "Perempuan"
  >("Laki-laki");
  const [formKategori, setFormKategori] = useState<"mahasiswa" | "siswa">(
    "mahasiswa",
  );
  ...
  // Modals state
  const [acceptTargetApp, setAcceptTargetApp] = useState<Application | null>(
    null,
  );
  const [rejectTargetApp, setRejectTargetApp] = useState<Application | null>(
    null,
  );
  ...
  const handleOpenEdit = (app: Application) => {
    ...
    setFormUniversitas(app.instansiPendidikan ?? app.universitas ?? "");
  ...
  const handleSaveCrud = (e: React.FormEvent) => {
    ...
    if (crudMode === "create") {
      const newApp: Application = {
        ...
        instansiPendidikan: formUniversitas,
  ...
  const finalAvailableYears = Array.from(
    new Set([...generatedYears, ...dbYears]),
  ).sort();
  ...
  const filteredHistoryApps = applications.filter((app) => {
    ...
    if (searchTerm) {
      ...
      const matchInstansi = (app.instansiPendidikan ?? app.universitas ?? "")
        .toLowerCase()
        .includes(term);
      const matchProdi = app.prodi?.toLowerCase().includes(term);
      if (!matchName && !matchId && !matchInstansi && !matchProdi) return false;
    }
    ...
    if (tahunFilter !== "All") {
      ...
      try {
        ...
        if (d) {
          ...
          const targetYear =
            tahunFilter === "custom" ? customTahunInput : tahunFilter;
  ...
  const handlePrintRiwayat = () => {
    ...
    const rowsHtml = filteredHistoryApps
      .map((app, index) => {
        const prodiText = app.prodi || "-";
        const startTgl = formatIndoDate(
          app.tanggalMulaiMagang || app.tanggalMulai,
        );
        const endTgl = formatIndoDate(
          app.tanggalSelesaiMagang || app.tanggalSelesai,
        );
        ...
        <tr>
          ...
          <td style="padding: 10px 8px; border-bottom: 1px solid #ddd;">${app.instansiPendidikan ?? app.universitas ?? "-"}</td>
  ...
  const handleExecuteReject = () => {
    ...
    const reason =
      rejectionReasonInput.trim() || "Berkas tidak memenuhi syarat";
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 space-y-6 shadow-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-slate-600">
                <div className="space-y-4">
                  <h5 className="font-bold text-slate-800 uppercase tracking-wider text-[10px] border-b border-slate-100 pb-2 flex items-center gap-1.5">
                    ...
                    Data Pribadi Calon Magang
                  </h5>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <span className="text-xs font-semibold text-slate-400 shrink-0 w-36">
                        Nama Lengkap:
                      </span>
              ...
              <div className="border-t border-slate-100 pt-6 space-y-3">
                <h5 className="font-bold text-slate-800 uppercase tracking-wider text-[10px] flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 bg-blue-500 rounded-full" /> Link
                  Google Drive Berkas Pendukung (Surat Pengantar & Rekomendasi)
                </h5>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
            <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-sm w-full p-6 space-y-6 text-center">
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => setAcceptTargetApp(null)}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
            <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-sm w-full p-6">
              <div className="text-center space-y-4">
                ...
                <h3 className="font-display font-black text-slate-900 text-base text-center">
                  Konfirmasi Tolak
                </h3>
                <p className="text-xs text-slate-500 text-center">
                  Berikan alasan penolakan untuk{" "}
                  <strong className="text-slate-800">
                    {rejectTargetApp.namaLengkap}
                  </strong>
                  :
                </p>
              </div>
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button
                  onClick={() => setRejectTargetApp(null)}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl"
                >
                  Batal
                </button>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1.5">
            Sistem monitoring terpusat seluruh data pendaftaran magang.
          </p>
        </div>
          ...
          <div className="bg-white border border-amber-200 rounded-3xl overflow-hidden shadow-xs shadow-amber-500/5">
            <div className="p-4 bg-amber-50/50 border-b border-amber-100 flex items-center justify-between px-6">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-display font-black text-sm text-amber-900 leading-none">
                    Pendaftaran Menunggu Persetujuan (Belum Diproses)
                  </h4>
                  <p className="text-[10px] text-amber-600 font-bold mt-1">
                    Total: {waitingHistoryList.length} Peserta Menunggu
                  </p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {waitingHistoryList.map((app) => (
                    ...
                    >
                      <td className="p-4 pl-6 font-mono text-[11px] font-bold text-slate-500">
                        {app.id}
                      </td>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">
                      Total: {appsInMonth.length} Peserta Magang
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 text-center whitespace-nowrap space-x-1">
                            ...
                            <button
                              onClick={() =>
                                handleDelete(app.id, app.namaLengkap)
                              }
                              title="Hapus Berkas"
                              className="p-2 bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white rounded-xl text-xs font-bold transition-all cursor-pointer inline-flex items-center justify-center shadow-xs"
                            >
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs">
            <div className="p-4 bg-slate-50 border-b border-slate-150 flex items-center justify-between px-6">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-display font-extrabold text-sm text-slate-900 leading-none">
                    Berkas Ditolak / Tidak Memenuhi Syarat
                  </h4>
                  <p className="text-[10px] text-slate-500 mt-1">
                    Total: {rejectedHistoryList.length} Pendaftaran
                  </p>
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {rejectedHistoryList.map((app) => (
                    ...
                    >
                      ...
                      <td
                        className="p-4 text-slate-500 italic max-w-xs truncate"
                        title={app.rejectionReason || app.statusNote}
                      >
                        {app.rejectionReason ||
                          app.statusNote ||
                          "Tidak disebutkan"}
        ...
        {sortedMonthKeys.length === 0 &&
          ...
          waitingHistoryList.length === 0 && (
            <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-xs">
              ...
              <h5 className="font-display font-extrabold text-base text-slate-800 leading-none">
                Tidak Ada Data Ditemukan
              </h5>
              <p className="text-xs text-slate-400 mt-2 max-w-sm mx-auto leading-relaxed">
                Silakan sesuaikan kriteria pencarian atau filter status dan
                bulan mulai magang Anda.
              </p>
            </div>
          )}
      </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="font-display font-black text-slate-900 text-base md:text-lg">
                {crudMode === "create"
                  ...
                  : `Ubah Data Pendaftar: ${editingAppId}`}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveCrud} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    required
                    value={
                      ["1 Bulan", "2 Bulan", "3 Bulan", "6 Bulan"].includes(
                        formDurasi,
                      )
                        ? formDurasi
                        : formDurasi
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700">
                  Link Google Drive Berkas Pendukung
                </label>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-sm w-full p-6 space-y-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500">
                ...
                ? Status akan diubah langsung menjadi{" "}
                <strong className="text-emerald-600">Lulus</strong>.
              </p>
            </div>
            <div className="flex justify-center gap-3">
              <button
              ...
              <button
                onClick={handleExecuteAccept}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-extrabold rounded-xl shadow-md shadow-emerald-500/10"
              >
                Ya, Terima
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-sm w-full p-6">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                onClick={handleExecuteReject}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-extrabold rounded-xl shadow-md shadow-rose-500/10"
              >
                Tolak
              </button>
            </div>
